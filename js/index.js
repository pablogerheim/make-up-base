...
let orderValue = 0
//let apiArray = []
let arrayFiltered = [...apiArray]
const fun = [byRating, byPrice, byPrice, byAlfa, byAlfa,]

//Função que vai percorer toda a API
function printsCards() {
  if (document.getElementById('catalog').children.length != 0) { document.getElementById('conteinerCards').remove() }
...
function rideFilters(item) {
  let booleanoB = false
  if (!brands.includes(item.brand)) { brands.push(item.brand), booleanoB = true }
  ...
  let booleanoT = false
  if (!product_types.includes(item.product_type)) { product_types.push(item.product_type), booleanoT = true }
...
function rideCard(arrayFiltered) {
  ...
  let conteudo = arrayFiltered.map((item) => `<div class="card" >
    ...
    <section class="product-description">
     ...
     <span class="product-brand background-price"> RS: ${formatValue(item.price)}</span></div>
    </section>
    <section class="product-details"><div class="details-row">
        ...
        <div>Price</div>
        <div class="details-bar">
          <div class="details-bar-bg" style="width= 250"> ${formatValue(item.price)}</div>
        </div>
      </div><div class="details-row">
        <div>Rating</div>
        <div class="details-bar">
...
function searchName(params) {
  setTimeout(() => {
    ...
    arrayFiltered = arrayFiltered.filter(item => item.name.toLowerCase().includes(params.toLowerCase()))
    printsCards()
  }, 100);
}
...
function updateArray() {
  arrayFiltered = [...apiArray]
  if (brand != "") { arrayFiltered = arrayFiltered.filter(item => item.brand === brand) }
  if (type != "") { arrayFiltered = arrayFiltered.filter(item => item.product_type === type) }
...
function byPrice(orderValue) {
  ...
  if (orderValue === '2') { arrayFiltered = arrayFiltered.reverse() }
}
...
function byAlfa(orderValue) {
  ...
  if (orderValue === '4') { arrayFiltered = arrayFiltered.reverse() }
}
...
function format() {
  ...
  arrayFiltered.forEach(item => { rideFilters(item) })
  printsCards()
}
